    ...
    $('tr').each(function (timeIdx) {
      ...
      $(this)
        ...
        .each(function (weekDayIdx) {
          ...
          const contents = $(this).html().split('<br>');
          const subject = contents[0]; // 첫 번째 항목은 과목명
          let teacher = '';
          let memo = null;

          if (contents.length === 2) {
            teacher = contents[1]; // 두 번째 항목은 강사명
          } else if (contents.length > 2) {
            teacher = contents.slice(1, -1).join(', '); // 두 번째부터 마지막 직전까지는 강사명
            memo = contents[contents.length - 1]; // 마지막 값은 memo
          }

          const moved = $(this).hasClass('변경'); // td의 클래스에 따라 moved 변수 설정
          const classid = subject+teacher;
          timetable[currentWeekDay][currentTime] = {
            //grade, 필요한거만 return하게 설정.
            //class: classNumber,
            weekday: weekDayIdx - 1,
            //weekdayString: $this._weekdayString[weekDayIdx],
            //classTime: currentTime + 1,
            //teacher, 일시적으로 사용하지 않음 (classID로 대체)
            //subject,
            classid,
            memo,
            moved,
